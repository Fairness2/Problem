...
class Neuron:
    ...
    def learn(self, img_file, right_name):
        ...
        # предел обучения для нейрона
        right_answer_f = 0.6

        anti_loop = 0
        answer = self.predict(img_file)
        # обучаем пока не достигнет нужного значения
        while (answer[0] != right_answer) or (answer[1] < right_answer_f and answer[0] == right_answer):

            cff = 1

            #разбираем две ситуации: ложно положительную и ложно отрицательную, в зависимости от ситуации меняется коэффициент
            if right_answer == False: cff = -1
